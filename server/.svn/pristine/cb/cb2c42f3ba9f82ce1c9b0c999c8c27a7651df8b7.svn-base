...
__modified_by__ = "jasonling@tencent.com"
...
class JceOutputStream(object):
    ...
    def __writeStruct(self, coder, tag, value):
        ...
        # print coder,tag,value

    def write(self, coder, tag, value):
        if coder.__taf_index__ == 999:
            self.__writeBoolean(tag, value)
        elif coder.__taf_index__ == 0:
            self.__writeInt8(tag, value)
        elif coder.__taf_index__ == 1:
            self.__writeInt16(tag, value)
        elif coder.__taf_index__ == 2:
            self.__writeInt32(tag, value)
        elif coder.__taf_index__ == 3:
            self.__writeInt64(tag, value)
        elif coder.__taf_index__ == 4:
            self.__writeFloat(tag, value)
        elif coder.__taf_index__ == 5:
            self.__writeDouble(tag, value)
        elif coder.__taf_index__ == 13:
            self.__writeBytes(tag, value)
        elif coder.__taf_index__ == 67:
            self.__writeString(tag, value)
        elif coder.__taf_index__ == 8:
            self.__writeMap(coder, tag, value)
        elif coder.__taf_index__ == 9:
            self.__writeVector(coder, tag, value)
        elif coder.__taf_index__ == 1011:
            self.__writeStruct(coder, tag, value)
        else:
            raise TafJceUnsupportType("taf unsupport data type:" % coder.__taf_index__)
    ...
    def __skipField(self, p):
        ...
        if p == DataHead.EN_INT8:
            ...
            if pi != DataHead.EN_INT8:
                raise TafJceDecodeInvalidValue("skipField with invalid type, type value: %d, %d." % (p, pi))
            size = self.__readInt32(0, True)
            ...
            raise TafJceDecodeMismatch("skipField with invalid type, type value:%d" % p)
    ...
    def __readInt8(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_ZERO:
                ...
                raise TafJceDecodeMismatch("read 'Char' type mismatch, tag: %d , get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readInt16(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_ZERO:
                ...
                raise TafJceDecodeMismatch("read 'Short' type mismatch, tag: %d , get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readInt32(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_ZERO:
                ...
                raise TafJceDecodeMismatch("read 'Int32' type mismatch, tag: %d, get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readInt64(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_ZERO:
                ...
                raise TafJceDecodeMismatch("read 'Int64' type mismatch, tag: %d, get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readString(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_STRING1:
                ...
                raise TafJceDecodeMismatch("read 'string' type mismatch, tag: %d, get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readBytes(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_BYTES:
                ti, pi, li = self.__readFrom()
                if pi != DataHead.EN_INT8:
                    raise TafJceDecodeMismatch("type mismatch, tag: %d, type: %d, %d" % (tag, p, pi))
                size = self.__readInt32(0, True)
                value, = struct.unpack_from(str(size) + 's', self.__buffer.buffer, self.__buffer.position)
                self.__buffer.position += size
                return value
            else:
                raise TafJceDecodeMismatch("type mismatch, tag: %d, type: %d" % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readFloat(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_ZERO:
                ...
                raise TafJceDecodeMismatch("read 'Float' type mismatch, tag: %d, get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readDouble(self, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_ZERO:
                ...
                raise TafJceDecodeMismatch("read 'Double' type mismatch, tag: %d, get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readStruct(self, coder, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p != DataHead.EN_STRUCTBEGIN:
                raise TafJceDecodeMismatch("read 'struct' type mismatch, tag: %d, get type: %d." % (tag, p))
    ...
    def __readMap(self, coder, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_MAP:
                ...
                raise TafJceDecodeMismatch("read 'map' type mismatch, tag: %d, get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
    ...
    def __readVector(self, coder, tag, require, default=None):
        if self.__skipToTag(tag):
            ...
            if p == DataHead.EN_LIST:
                ...
                raise TafJceDecodeMismatch("read 'vector' type mismatch, tag: %d, get type: %d." % (tag, p))
        elif require:
            raise TafJceDecodeRequireNotExist("require field not exist, tag: %d" % tag)
        return default
